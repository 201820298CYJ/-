...
def insert():   #添加功能
    ...
    value = (id, name, age, home, identity,grade,sex)
    insert_sql = '''INSERT INTO list(id,name, age,home,identity,grade,sex) values (%s,%s,%s,%s,%s,%s,%s)'''
    cursor = get_cursor()  # 打开数据库
...
def update():   #更新
    ...
    grade = request.form.get("grade")
    sex = request.form.get("sex")
    value = ( name, age, home, identity,grade,sex,id)
    cursor = get_cursor()  # 打开数据库
    cursor.execute("update list set name = %s,age = %s,home =%s,identity = %s,grade = %s,sex = %s  where id = %s", value)  # 删除某一行值
    msg="编辑成功！"
    return msg
